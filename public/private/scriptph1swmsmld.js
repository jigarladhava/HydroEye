document.addEventListener('DOMContentLoaded', async function () {
  ...
  locationsDropdown.addEventListener('change', async function () {
    ...
    if (selectedLocation.length > 0) {
      const responseInstruments = await fetch(`/ph1/tags/${selectedLocation}`);
      let instrumentsData = await responseInstruments.json();
      instrumentsData = instrumentsData.filter((tagName) => tagName.includes('_FT') || tagName.includes('_FTE'));
  ...
  form.addEventListener('submit', async function (event) {
    ...
    try {
      const response = await fetch(`/ph1/swms/MLDdata?objectname=${tagName}&startTime=${startTime}&endTime=${endTime}`);
      ...
      Highcharts.chart('chartContainer1', {
        ...zoomOptions,
        chart: {
          type: 'column' // Use column chart type for bar chart
        },
