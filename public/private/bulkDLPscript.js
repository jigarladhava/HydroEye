    ...
    try {
        ...
        Highcharts.chart('chartContainer1', {
            ...
            series: [{
                cropThreshold: 9999,
                name: data[0].tagName,
                data: data.filter(item => item.tagName.endsWith('.AI1') || item.tagName.endsWith('.BI1') || item.tagName.endsWith('.CI1'))
        ...
        Highcharts.chart('chartContainer2', {
            ...
            series: [{
                cropThreshold: 9999,
                name: data[0].tagName,
                data: data.filter(item => item.tagName.endsWith('.AI2') || item.tagName.endsWith('.BI2') || item.tagName.endsWith('.CI2'))
                    .map(item => { return [ new Date(item.timestamp).getTime() + 19800000, parseFloat(item.value)] })
            }]
        });
        ...
        Highcharts.chart('chartContainer3', {
            ...
            series: [{
                cropThreshold: 9999,
                name: data[0].tagName,
                data: data.filter(item => item.tagName.endsWith('.AI3') || item.tagName.endsWith('.BI3') || item.tagName.endsWith('.CI3'))
                    .map(item => { return [ new Date(item.timestamp).getTime() + 19800000, parseFloat(item.value)] })
            }]
        });
